...
<!-- wp:group {"className":"slateframe-project-card","layout":{"type":"constrained"}} --><div class="wp-block-group slateframe-project-card"><!-- wp:post-featured-image {"isLink":true,"aspectRatio":"4/3"} /--><!-- wp:post-title {"isLink":true,"fontSize":"large"} /--><!-- wp:post-excerpt {"moreText":""} /--><!-- wp:post-date {"format":"M Y"} /--></div><!-- /wp:group -->
<!-- /wp:post-template --><!-- wp:query-no-results --><!-- wp:paragraph --><p><?php echo esc_html_x( 'Publish some work to populate this project grid.', 'Empty project grid message', 'slateframe' ); ?></p><!-- /wp:paragraph --><!-- /wp:query-no-results -->
</div><!-- /wp:query -->
</div><!-- /wp:group -->
